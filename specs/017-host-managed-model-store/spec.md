...
- **FR-505**: `crates/nexus-backend-runtimes/src/models_store/` MUST be added with submodules: `mod.rs` (public API), `install/` (`install_model`, `uninstall_model`, dedup, private-flag handling — split into `dto.rs`, `fetcher.rs`, `dedup.rs`, `pipeline.rs` to stay under the per-file LOC cap), `download.rs` (semaphore-guarded download + `Range`-aware resume + checksum verification), `blobs.rs` (content-addressed blob store under `$root/blobs/<sha[0:2]>/<sha>`, hardlink/symlink materialization, `gc_blobs`; US10b), `quantization.rs` (re-exports `nexus_protocol::Quantization`; US10a), `leases.rs` (`acquire_lease`, `release_lease`, `list_active_leases`, VRAM accounting), `resolver/` (`check_model_dependencies`, tie-breaker, `resolve_dry_run` for US10c), `provenance.rs` (license + source metadata helpers + `HfProbe` trait), `reclaim.rs` (private-install lifecycle sweeper), `verify.rs` (post-install re-hash), `errors.rs` (`ModelStoreError`). Every submodule ≤ 350 LOC (Principle III). Schema DDL lives in top-level `migrations/NNN_*.sql` files per repo convention — no `schema.rs` inside `models_store/`.
- **FR-506**: `ModelDependency` + `Quantization` are canonical in `nexus_protocol` (light shared crate) and re-exported by both `nexus-backend-runtimes::models_store` and `nexus-extension::manifest`. `ManifestParse` error is reused for malformed blocks.
...
- **SC-504**: `grep -RE "\binstalls_store\b" crates/nexus-backend-runtimes/src/ crates/nexus-api/src/ | grep -vE "runtime_installs_store|models_store|llamacpp/installs_store|llamacpp::installs_store"` returns 0 after Phase J (shim removed). The `llamacpp/installs_store` submodule is an unrelated llama.cpp-internal store and is explicitly excluded from the SC-504 grep.
