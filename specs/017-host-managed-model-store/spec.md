...
- **Not** implementing a GUI for model-store administration. HTTP surface only; UI consumption is a downstream spec.
...
model_dependencies:
  - family: llama-cpp-gguf            # required, string
    version: llama-3-8b-instruct      # required, string (semver OR free-form identifier)
    min_params: 7B                    # optional, parsed as a parameter-count threshold
    quantization: Q4_K_M              # optional, string; absent = "any quantization acceptable"
...
- **FR-505**: `crates/nexus-backend-runtimes/src/models_store/` MUST be added with submodules: `mod.rs` (public API), `schema.rs` (table DDL), `install.rs` (`install_model`, `uninstall_model`, dedup, private-flag handling), `download.rs` (semaphore-guarded download + `Range`-aware resume + checksum verification), `leases.rs` (`acquire_lease`, `release_lease`, `list_active_leases`, VRAM accounting), `resolver.rs` (`check_model_dependencies`, tie-breaker), `provenance.rs` (license + source metadata helpers), `errors.rs` (`ModelStoreError`). Every submodule ≤ 350 LOC (Principle III).
...
- **`ModelDependency`** (new, `nexus-extension::manifest`): `{ family, version, min_params: Option<ParamCount>, quantization: Option<String>, variant: Option<String>, required: bool }`.
