...
- [ ] T010 [P] [US4] Add Mixtral-shaped + dense GGUF fixtures (small synthetic headers, ~4 KB each) under `crates/nexus-model-metadata/tests/fixtures/` (`mixtral_8x7b_header.gguf`, `llama_dense_header.gguf`)
- [ ] T011 [P] [US4] Test `gguf_extract_marks_mixtral_fixture_as_moe` in `crates/nexus-model-metadata/tests/moe_extraction.rs` — fixture-driven; asserts `is_moe == Some(true)` AND `expert_layer_count == Some(<n>)` (FR-008)
- [ ] T012 [P] [US4] Test `gguf_extract_marks_dense_fixture_as_not_moe` in `crates/nexus-model-metadata/tests/moe_extraction.rs` — asserts `is_moe == Some(false)` AND `expert_layer_count == None` (FR-008)
- [ ] T013 [P] [US4] Test `gguf_extract_arch_name_fallback_marks_qwen3moe_as_moe` in `crates/nexus-model-metadata/tests/moe_extraction.rs` — fixture omits `expert_count`, has `general.architecture = "qwen3moe"`; asserts `is_moe == Some(true)` (research.md R2)
- [ ] T014 [US4] Test `installed_artifacts_migration_021_is_additive_and_reversible` in `crates/nexus-models-store/tests/migrations.rs` — runs migration 021 forward then reverse against in-memory SQLite; asserts both columns appear / disappear; existing rows preserved (FR-008)
- [ ] T015 [US4] Test `installed_artifact_first_read_with_null_is_moe_triggers_reprobe` in `crates/nexus-models-store/tests/reprobe.rs` — inserts a row with `is_moe = NULL` AND `extraction_status = 'success'`; reads twice; asserts second read sees populated columns (FR-007)
- [ ] T016 [US4] Contract test `available_model_dto_includes_is_moe_and_expert_layer_count` in `extensions/builtin/local-llm/rust/tests/available_models_contract.rs` — JSON-shape assertion against `contracts/available_model.schema.json` (FR-006, FR-011)

### Implementation for User Story 4

- [ ] T017 [US4] Create migration `migrations/021_installed_artifact_moe_metadata.sql` adding `is_moe INTEGER` and `expert_layer_count INTEGER` (both nullable, default NULL) to `model_store_installed_artifacts` per `data-model.md` § Schema delta
- [ ] T018 [US4] Extend `ExtractedMetadata` struct in `crates/nexus-model-metadata/src/model.rs` with `is_moe: Option<bool>` and `expert_layer_count: Option<u32>` per `data-model.md`
- [ ] T019 [US4] Extend `extract` impl in `crates/nexus-model-metadata/src/gguf.rs` to read `<arch>.expert_count` and apply the curated `MOE_ARCHITECTURES` fallback set (`mixtral`, `qwen2moe`, `qwen3moe`, `deepseek2`, `dbrx`, `gptoss`, `glm4_moe`); set `expert_layer_count = layer_count` when `is_moe == Some(true)` per `data-model.md` § Extraction algorithm + research.md R2/R3
- [ ] T020 [US4] Extend `InstalledArtifact` row struct in `crates/nexus-models-store/src/model.rs` with the two new optional columns per `data-model.md` § InstalledArtifact
- [ ] T021 [US4] Extend the install pipeline writer in `crates/nexus-models-store/src/install/` to persist `is_moe` and `expert_layer_count` from `ExtractedMetadata` (additive — no signature change for existing callers)
- [ ] T022 [US4] Implement idempotent re-probe in the read path of `crates/nexus-models-store/src/model.rs` (or the appropriate read module) — when a row has `is_moe IS NULL` AND `extraction_status = 'success'`, fire-and-forget re-extract via `nexus_model_metadata::extract_any` and persist the result via `UPDATE` (FR-007, research.md R7)
- [ ] T023 [US4] Extend `AvailableModelDto` in `extensions/builtin/local-llm/rust/src/chat/handlers.rs` with `is_moe: bool` and `expert_layer_count: Option<u32>`; map `Option<bool>` from the host row to `bool` (NULL → false) per `data-model.md` § AvailableModelDto Source
- [ ] T024 [US4] Update the `list_available_models` query in `extensions/builtin/local-llm/rust/src/chat/handlers.rs` to select the two new columns; ensure SELECT list and DTO field order match
- [ ] T025 [US4] Run `cargo test -p nexus-model-metadata --test moe_extraction` — all four tests green
- [ ] T026 [US4] Run `cargo test -p nexus-models-store --test migrations --test reprobe` — both tests green
- [ ] T027 [US4] Run `cargo test --manifest-path extensions/builtin/local-llm/rust/Cargo.toml --test available_models_contract` — green
