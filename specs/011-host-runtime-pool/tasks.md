...
- [X] T034 [US2] Moved `manifest/{version,install,schemas}.rs` to `crates/nexus-backend-runtimes/src/manifest/` with new `mod.rs`. Verified: `ls crates/nexus-backend-runtimes/src/manifest/` shows all four files.
- [X] T035 [US2] Moved `crates/nexus-local-llm/src/llamacpp/*` to `crates/nexus-backend-runtimes/src/llamacpp/{mod,install_pipeline,probe,installs_store,channel_builder}.rs`. `installs_store.rs` rewritten to target `host_runtime_installs`. P1 schema switch landed.
