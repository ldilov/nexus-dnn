...
//!
//! Fans the generic `runtime.release_memory` JSON-RPC method across every
//! live lease handle, regardless of which extension owns it. Each worker
//! unloads its model and runs `gc + torch.cuda.empty_cache`. Per-worker
//! errors / timeouts are logged and skipped — partial success still
//! returns 200 so one wedged worker can't block reclaiming the rest.
