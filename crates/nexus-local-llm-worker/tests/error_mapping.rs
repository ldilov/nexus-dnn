...
fn stable_codes_cover_all_worker_error_variants() {
    ...
    let cases = [
        ...
        BackendBusy,
        ModelMissing { model_id: "m".into() },
        ModelMetadataIncomplete { field: "f".into() },
        RuntimeMissing { install_id: "r".into() },
        IncompatibleRuntime { format: "GGUF".into() },
        Cancelled,
        CapabilityUnavailable { capability: "embeddings".into() },
        CorpusNotFound { corpus_id: "c".into() },
        PoolSlotNotFound { key: "k".into() },
        RestartDenied { key: "k".into(), reason: "r".into() },
        LeaseNotFound { lease_id: "l".into() },
        LeaseAcquireTimeout { install_id: "r".into() },
...
fn retry_safe_partitions_as_expected() {
    ...
    assert!(LeaseAcquireTimeout { install_id: "x".into() }.retry_safe());

    assert!(!ModelMissing { model_id: "x".into() }.retry_safe());
    assert!(!RuntimeMissing { install_id: "x".into() }.retry_safe());
    assert!(!CapabilityUnavailable { capability: "x".into() }.retry_safe());
