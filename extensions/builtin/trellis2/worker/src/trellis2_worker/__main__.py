...
def cli() -> int:
    ...
    profile = os.environ.get("NEXUS_3D_TRELLIS2_RUNTIME", "fake")
    if profile != "fake":
        try:
            import torch  # noqa: F401
        except ImportError as e:
            print(
                '{"level":"warn","target":"nexus.3d.trellis2",'
    ...
    elif profile == "gb10-flash":
        try:
            from .pipeline_trellis2 import register_trellis2_handlers
        except ImportError as e:
            worker.logger.error("pipeline import failed", profile=profile, error=str(e))
            return 2
        register_trellis2_handlers(worker)
    else:
