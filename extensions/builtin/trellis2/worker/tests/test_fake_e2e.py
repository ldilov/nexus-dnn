...
from trellis2_worker.rpc import Methods, Notifications
...
def _base_refine_params(tmp_path: Path, **overrides) -> dict:
    out = tmp_path / "refine" / "mesh.glb"
    params = {
        "mesh_path": "in.glb",
        "image_path": "x.png",
        "output_path": str(out),
...
def _drive_refine(tmp_path: Path, params: dict):
    worker = Worker(profile="fake")
    notes: list[tuple[str, dict]] = []
    ...
    worker.emit_notification = _capture  # type: ignore
    register_fake_handlers(worker)
    handler = worker._handlers[Methods.REFINE_START]
    result = asyncio.new_event_loop().run_until_complete(handler(params))
    return result, notes


def test_fake_refine_produces_glb_and_metadata(tmp_path: Path):
    result, _ = _drive_refine(tmp_path, _base_refine_params(tmp_path))
...
def test_fake_refine_emits_ordered_progress_then_done(tmp_path: Path):
    _, notes = _drive_refine(tmp_path, _base_refine_params(tmp_path))
    methods = [m for m, _ in notes]
    ...
    stages = [p["stage"] for m, p in notes if m == Notifications.PROGRESS]
    assert stages[0] == "start"
    for expected in ("encode", "shape", "texture", "decode", "mesh", "glb"):
        assert expected in stages, expected


def test_fake_refine_progress_steps_match_param_counts(tmp_path: Path):
    _, notes = _drive_refine(
        tmp_path, _base_refine_params(tmp_path, shape_steps=5, texture_steps=4)
    )
    shape = [p for m, p in notes if m == Notifications.PROGRESS and p["stage"] == "shape"]
    texture = [p for m, p in notes if m == Notifications.PROGRESS and p["stage"] == "texture"]
    assert len(shape) == 5
    assert len(texture) == 4


def test_fake_refine_cancels_cooperatively_and_writes_no_glb(tmp_path: Path):
    params = _base_refine_params(tmp_path)
    notes: list[tuple[str, dict]] = []
    ...
    with pytest.raises(GenerationCancelled):
        asyncio.new_event_loop().run_until_complete(
            refine_fake_e2e(params, _emit, cancel_event=cancel_event)
        )
...
def test_fake_refine_missing_mesh_rejected(tmp_path: Path):
    from trellis2_worker.params import validate_refine_params
