...
The worker is a compiled Rust sidecar at [`crates/nexus-local-llm-worker`](../../../crates/nexus-local-llm-worker/).

- `runtime.family` in `manifest.yaml` is `native`; `runtime.entrypoint` points at the compiled worker binary (`bin/nexus-local-llm-worker${exe_suffix}`).
- The extension does not spawn `llama-server`. For host-registered runtimes (llama.cpp), it calls `host.runtimes.acquire_lease(...)` and proxies OpenAI-compatible HTTP to `lease.channel.base_url`. The host spawns, supervises, and tears down the runtime child.
- Chat session, RAG corpus, and migration SQL (migrations `001`–`005`) are preserved from the earlier Python worker; no user-data migration required.

## Runtime ownership split

This extension does not own the runtime install lifecycle. The host crate `nexus-backend-runtimes` owns:
