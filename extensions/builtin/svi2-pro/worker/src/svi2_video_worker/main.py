...
Worker process talks to a Rust shim (svi2-pro video worker):
...
def base_release_memory() -> dict[str, int]:
    ...
    loaded (freed_mb == 0). svi2 builds + drops expert models per render,
    so this generic sweep IS the full reclaim — there is no long-lived
    resident pipeline to unload.
    """
...
class Worker:
    ...
    def _register_intrinsic(self) -> None:
        async def handshake(_params: Any) -> dict[str, Any]:
            return {
                "runtime_id": f"nexus.video.svi2-pro.{self.profile}",
