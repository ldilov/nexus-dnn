...
def _check_is_instance(model, module_class):
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x, feat_cache=None, feat_idx=[0]):
        ...
        for layer in self.residual:
            if _check_is_instance(layer, CausalConv3d) and feat_cache is not None:
                idx = feat_idx[0]
                cache_x = x[:, :, -CACHE_T:, :, :].clone()
...
class Encoder3d(nn.Module):
    ...
    def forward(self, x, feat_cache=None, feat_idx=[0]):
        ...
        for layer in self.middle:
            if _check_is_instance(layer, ResidualBlock) and feat_cache is not None:
                x = layer(x, feat_cache, feat_idx)
            else:
                x = layer(x)
...
class WanVideoVAE(nn.Module):
    ...
    def _tiled_decode(self, hidden_states, device, tile_size, tile_stride):
        ...
        for h, h_, w, w_ in tasks:
            ...
            batch = self.model.decode(batch, self.scale).to(data_device)
    ...
    def encode(self, videos, device, tiled=False, tile_size=(34, 34), tile_stride=(18, 16)):
        ...
        for video in videos:
            ...
            if tiled:
                ts = (tile_size[0] * self.upsampling_factor, tile_size[1] * self.upsampling_factor)
                tst = (tile_stride[0] * self.upsampling_factor, tile_stride[1] * self.upsampling_factor)
                hidden_state = self._tiled_encode(video, device, ts, tst)
            else:
    ...
    def decode(self, hidden_states, device, tiled=False, tile_size=(34, 34), tile_stride=(18, 16)):
        ...
        for hidden_state in hidden_states:
            ...
            if tiled:
                video = self._tiled_decode(hidden_state, device, tile_size, tile_stride)
            else:
