...
A worker process-crash root cause: a child (ffmpeg, sd.cpp, RIFE) that inherits
fd 1 prints into the NDJSON stream and the host framer kills the lease. After
`_hijack_stdout`, fd 1 is stderr and only the stashed RPC handle reaches stdout.
"""
...
def test_raw_fd1_write_goes_to_stderr_not_rpc_wire():
    snippet = (
        "import os,sys;"
        "from svi2_video_worker.__main__ import _hijack_stdout;"
        "_hijack_stdout();"
