...
Worker process talks to a Rust shim (longcat-video worker):
...
def base_release_memory() -> dict[str, int]:
    ...
    loaded (freed_mb == 0). The render profile overrides this to evict the
    resident pipeline first, then calls this for the final sweep.
    """
...
class Worker:
    ...
    def _register_intrinsic(self) -> None:
        async def handshake(_params: Any) -> dict[str, Any]:
            return {
                "runtime_id": f"nexus.video.longcat.{self.profile}",
