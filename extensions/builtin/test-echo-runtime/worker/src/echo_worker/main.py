"""Spec 032 — JSON-RPC over stdio echo worker.

Implements the four host-mandated methods:

- handshake  → returns protocol_version, worker_version, accepts_methods
- health     → returns {ok: true}
- shutdown   → returns {} and exits the process
- echo       → returns {text: <params.text verbatim>}

The frame format is one JSON object per line (NDJSON), matching the
host's framer in `nexus_backend_runtimes::generic::leases::framer`.
