"""Stdio NDJSON JSON-RPC event loop for the faceavatar identity-head worker.

Worker process talks to a Rust shim (faceavatar-extension):
...
def base_release_memory() -> dict[str, int]:
    ...
    Torch-absent-safe (returns zeros, never raises). The faceavatar pipeline
...
class Worker:
    ...
    def _register_intrinsic(self) -> None:
        async def handshake(_params: Any) -> dict[str, Any]:
            return {
                "runtime_id": f"nexus.3d.faceavatar.{self.profile}",
